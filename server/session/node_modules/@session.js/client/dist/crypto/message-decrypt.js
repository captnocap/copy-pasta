import { removeMessagePadding } from '../crypto/message-padding.js';
...
import { Uint8ArrayToHex, base64ToUint8Array, concatUInt8Array, removePrefixIfNeeded } from '../utils.js';
...
*/
export function decodeMessage(body, options) {
...
export function decryptMessage(keypairs, envelope) {
 ...
 */
export function decryptEnvelopeWithOurKey(keypair, envelope) {
    try {
...
function decryptForClosedGroup(encryptionKeyPairs, envelope) {
    try {
 ...
 */
export function decryptWithSessionProtocol(keypair, envelope, isClosedGroup) {
    ...
    const plaintextWithMetadata = sodium.crypto_box_seal_open(envelope.content, recipientX25519PublicKey, new Uint8Array(keypair.x25519.privateKey));
    ...
    const isValid = sodium.crypto_sign_verify_detached(signature, concatUInt8Array(plaintext, senderED25519PublicKey, recipientX25519PublicKey), senderED25519PublicKey);
    if (!isValid) {
