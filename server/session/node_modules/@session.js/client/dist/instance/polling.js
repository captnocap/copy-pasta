...
import { Poller } from '../polling/index.js';
...
import { mapDataMessage, mapReactionMessage, mapUnsendMessage, mapReceiptMessage, mapTypingMessage, mapScreenshotTakenMessage, mapMediaSavedMessage, mapMessageRequestResponseMessage, mapCallMessage, } from '../messages/index.js';
export function addPoller(poller) {
    if (!(poller instanceof Poller))
        throw new SessionValidationError({ code: SessionValidationErrorCode.InvalidPoller, message: 'Poller must be an instance of Poller' });
    ...
    poller._attachedToInstance(this, {
        onMessagesReceived: async (messages) => {
            ...
            const reactionMessages = newMessages
                ...
                .filter(m => m.content.dataMessage?.reaction);
            reactionMessages
                .filter(m => m.content.dataMessage?.reaction?.action ===
        ...
        updateLastHashes: async (hashes) => {
            ...
            if (lastHashes !== null) {
                newLastHashes = JSON.parse(lastHashes);
            }
            hashes.forEach(h => {
