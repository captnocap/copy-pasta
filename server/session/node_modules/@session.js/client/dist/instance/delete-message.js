...
import { UnsendMessage } from '../messages/schema/delete-message.js';
...
export async function deleteMessage({ to, timestamp, hash }) {
 ...
 * retrieved them yet */
async function deleteMessagesFromSwarm({ hashes }) {
    const keypair = this.keypair;
...
*/
async function propagateUnsendMessages({ messages }) {
    const unsendMessages = messages.map(m => new UnsendMessage({
    ...
    const wrappedMessages = await wrap(this.keypair, rawMessages.map((rawMessage, i) => {
        return {
    ...
    const wrappedSyncMessages = await wrap(this.keypair, unsendMessages.map((m, i) => {
        return {
